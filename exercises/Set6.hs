...
-- constraint 'Price a' says that this only works for the members of the Price class
...
class Cycle a where
    step :: a -> a
    stepMany :: Int -> a -> a
    stepMany n x = if n == 0 then x
                    else stepMany (n-1) (step x)
