...
member x (Set u) = length (filter (==x) u) > 0

-- add a member to a set
add :: Eq a => Ord a => a -> Set a -> Set a
add x (Set xs) = if member x (Set xs) then Set xs
                else Set (sort (x:xs))
...
instance Semigroup Distance where
    (Distance x) <> (Distance z) = Distance (x+z)

instance Semigroup Time where
    (Time x) <> (Time z) = Time (x+z)

instance Semigroup Velocity where
    (Velocity x) <> (Velocity z) = Velocity (x+z)
...
passwordAllowed s (MinimumLength k) = length s >= k

passwordAllowed [] (ContainsSome x) = False
passwordAllowed s (ContainsSome []) = False
passwordAllowed (s:ss) (ContainsSome x) = length (filter (==s) x) > 0 || passwordAllowed ss (ContainsSome x)

passwordAllowed s (DoesNotContain x) = not (passwordAllowed s (ContainsSome x))

passwordAllowed s (And pwr pwr') = passwordAllowed s pwr && passwordAllowed s pwr'
passwordAllowed s (Or pwr pwr')  = passwordAllowed s pwr || passwordAllowed s pwr'
...
evaluate (Operation "*" x y) = evaluate x * evaluate y
evaluate (Operation "-" x y) = evaluate x - evaluate y
evaluate (Operation "/" x y) = div (evaluate x) (evaluate y)

render :: Arithmetic -> String
render (Literal x) = show x
render (Operation s x y) = "(" ++ render x ++ s ++ render y ++ ")"
