...
oddFactorial 1 = 1
oddFactorial x = if even x then oddFactorial (x-1) else x * oddFactorial (x-1)
...
leftpad str k = if length str < k then leftpad (" " ++ str) k
                else str
...
smallestDivisor x = findSmallestDivisor x 2
                    where findSmallestDivisor n k = if mod n k == 0 then k
                          else findSmallestDivisor n (k+1)
