...
get :: UpDown -> Int
get (MkUp count) = count
get (MkDown count) = count

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (MkUp count) = MkUp (count+1)
tick (MkDown count) = MkDown (count-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUp count) = MkDown count
toggle (MkDown count) = MkUp count
...
-- could have used zipWith for this one
...
toList Empty = []
toList (Pair key value keyvals) = (key,value) : toList keyvals

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList (head:rest) = Pair (fst head) (snd head) (fromList rest)
...
toNat :: Int -> Maybe Nat
toNat 0 = Just Zero
toNat x = if x > 0 then Just (PlusOne (maybe Zero id (toNat (x-1))))
            else Nothing
...
toBin 0 = O End
toBin 1 = I End
-- toBin x = if mod x 2 == 1 then I (toBin (div x 2))
--           else O (toBin (div x 2))

-- more elegant?
toBin x = convert2Bit (mod x 2) (toBin (div x 2))
            where convert2Bit z = if z == 1 then I else O
