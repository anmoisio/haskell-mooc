...
firstHalf str = take (halfCeiling str) str
                where halfCeiling s = if even (length s) then div (length s) 2
                                        else div (length s) 2 + 1
...
-- sumRights (x:xs) = sum (map (either (+0) (+ (sumRights xs))) xs)
...
updateXY :: String -> [Integer] -> [Integer]
updateXY command xy
    | command == "up" = head xy : [last xy + 1]
    | command == "down" = head xy : [last xy - 1]
    | command == "right" = (head xy +1) : [last xy]
    | command == "left" = (head xy -1) : [last xy]

printXY :: String -> [Integer] -> String
printXY command xy
    | command == "printX" = show $ head xy
    | command == "printY" = show $ last xy

createList :: [String] -> [Integer] -> [String] -> [String]
createList result loc coms
  | null coms = result
  | head (head coms) == 'p'
        = createList (result ++ [printXY (head coms) loc]) loc (tail coms)
  | otherwise = createList result (updateXY (head coms) loc) (tail coms)

interpreter :: [String] -> [String]
interpreter = createList [] [0, 0]
